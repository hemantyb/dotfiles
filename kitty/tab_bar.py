...
pink = as_rgb(int("FF9E64", 16))
truepink = as_rgb(int("F7768E", 16))
...
def draw_tab_with_separator(
    ...
    if is_last:
        ...
        cwd = truncate_str(get_cwd_icon() + draw_data.sep, remaining_size)
...
def get_cwd_icon():
    ...
    if tab_manager is not None:
        window = tab_manager.active_window
        if window is not None:
            cwd = window.cwd_of_child or os.getenv("HOME", "/")
